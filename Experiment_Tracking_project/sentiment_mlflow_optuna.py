...
def objective_lr(trial):
    ...
    pipeline.set_params(
        tfidf__max_features=trial.suggest_int("max_features", 2000, 5000, step=500),
...
def objective_nb(trial):
    ...
    pipeline.set_params(
        tfidf__max_features=trial.suggest_int("max_features", 3000, 10000, step=1000),
        tfidf__min_df=trial.suggest_int("min_df", 3, 7),
        tfidf__max_df=trial.suggest_float("max_df", 0.7, 0.9)
    )
...
for model_name, obj_fn in objectives.items():
    ...
    with mlflow.start_run(run_name=model_name):
        ...
        results[model_name] = (study.best_value, train_f1, test_f1)
...
for model, (cv, tr, te) in results.items():
    print(f"{model} | CV={cv:.4f} | Train={tr:.4f} | Test={te:.4f}")
